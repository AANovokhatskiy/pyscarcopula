...
def iter_forward_weight_block_arrays(
        grid, u, copula, x_grid=None, block_size=None,
        max_elements=2_000_000, emission_block=None, element_width=1):
    """Yield forward predictive weights and emissions one row block at a time.

    This is the block-array counterpart of ``iter_forward_weight_blocks``.  It
    keeps the same streaming state update, but returns ``weights_block`` with
    shape ``(block_rows, grid.K)`` so callers can fuse row-wise postprocessing
    without entering Python once per row and dimension.
    """
    u = np.asarray(u, dtype=np.float64)
    n = len(u)
    if x_grid is None:
    ...
    for start in range(0, n, block_size):
        ...
        weights_block = np.empty(expected_shape, dtype=np.float64)
        for local, k in enumerate(range(start, stop)):
            weights_block[local] = grid.predictive_weights_from_phi(phi)
            if k < n - 1:
                phi = grid.advance_forward_phi(phi, fi_block[local])

        yield start, stop, weights_block, fi_block, u_block
