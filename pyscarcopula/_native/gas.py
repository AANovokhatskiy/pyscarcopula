...
def require_available() -> None:
    """Raise ``NativeUnavailable`` unless the compiled evaluator can load."""
    _extension.load()


def ensure_supported(copula) -> None:
    """Raise ``NativeUnsupported`` for copulas outside the native GAS set."""
    _descriptors.ensure_supported_for_gas(copula)


def supported(copula) -> bool:
    """Return whether ``copula`` has an exact native GAS implementation."""
    return _descriptors.supported_for_gas(copula)
