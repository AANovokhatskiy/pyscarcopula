...
void copula_pdf_and_grad_grid_precomputed(
    ...
    fi.assign(static_cast<std::size_t>(n_obs) * K, 0.0);
    dfi_dx.assign(static_cast<std::size_t>(n_obs) * K, 0.0);
...
void copula_fi_row_on_grid(
    ...
    double v1 = 0.0;
    double v2 = 0.0;
    apply_rotation(u[2 * t], u[2 * t + 1], static_cast<int>(spec.rotation), v1, v2);
    for (std::size_t j = 0; j < x_grid.size(); ++j) {
