...
int evaluate_log_pdf_rows(
    ...
    if (observation_rows < 0 || observation_columns != plan.dimension
        || (observation_rows > 0 && log_pdf == nullptr)) {
...
DensityResult log_pdf_rows(
    ...
    if (n_threads <= 0 || observation_rows < 0
        ...
        fail_density(out, SCAR_INVALID_SIZE, -1, -1, -1);
        return out;
    }
    ...
    if (parameter_status != SCAR_OK) {
        fail_density(out, parameter_status, -1, -1, -1);
        return out;
    }
    ...
    if (prepare_status != SCAR_OK) {
        fail_density(out, prepare_status, -1, -1, -1);
        return out;
    }
    ...
    const int status = detail::evaluate_log_pdf_rows(
        plan,
        prepared_edges,
        parameters,
        observations,
