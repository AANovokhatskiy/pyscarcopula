"""Capture and compare C++ architecture-refactor benchmarks.
...
DEFAULT_MANIFEST = ROOT / "benchmarks" / "cpp_refactor_manifest.json"
DEFAULT_OUTPUT = ROOT / "benchmark_artifacts" / "cpp_refactor_baseline.json"
TESTS = ROOT / "tests"
...
def _scar_fixture(case: dict[str, Any], threads: int):
    ...
    return observations, ClaytonCopula(rotate=180), AutoTMConfig(**kwargs)
...
def _prepare_scar_ou(case: dict[str, Any], threads: int) -> PreparedCase:
    ...
    def call():
        ...
        return _cpp_scar_ou.neg_loglik_with_grad_info(
            1.2, 0.1, 0.7, observations, copula, config)
...
def _prepare_scar_ou_prepared(
    ...
    def call():
        return evaluator.neg_loglik_with_grad_info(1.2, 0.1, 0.7)
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "reference runner. 'physical' resolves to the pool size."
        ),
    )
    parser.add_argument(
        "--compare-to",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.cpu_set:
        try:
            cpu_pool = [int(value) for value in args.cpu_set.split(",")]
        except ValueError as exc:
            raise SystemExit("--cpu-set must contain comma-separated integers") from exc
        logical = max(1, os.cpu_count() or 1)
        if (
                not cpu_pool
    ...
    payload = {
        "schema_version": 3,
        "artifact_type": "cpp-refactor-benchmark-capture",
